...
- **Python 3.10+** (uv поставит сам, если нет)
- **ffmpeg** в `PATH` - конвертация FLAC в MP3 и подготовка обложек
- **JS-рантайм** для YouTube: **Deno** (рекомендуется), Node.js, Bun или QuickJS.
  Современному `yt-dlp` он нужен для полноценной поддержки YouTube; проект сам находит
  установленный рантайм и тащит решатель челленджей `yt-dlp-ejs`
- **Docker** или бинарник `telegram-bot-api` - опционально, только для отправки файлов больше 50 МБ через локальный Bot API server
